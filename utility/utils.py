...
def get_source_news(prefix,fold):
    role = json.load(open('./{}/{}/text_role.json'.format(prefix,fold)))
    source = role["source news"]
    return source

def load_text_data(prefix, fold, normalize=True):
    """
    ...
    """
    adj_full = scipy.sparse.load_npz('./{}/text_adj_full.npz'.format(prefix)).astype(np.bool)
    adj_train = scipy.sparse.load_npz('./{}/{}/text_adj_train.npz'.format(prefix,fold)).astype(np.bool)
    role = json.load(open('./{}/{}/text_role.json'.format(prefix,fold)))
    feats = np.load('./{}/text_feats.npy'.format(prefix))
...
def load_user_data(prefix, fold, normalize=True):
    user_adj_full = scipy.sparse.load_npz('./{}/user_adj_full.npz'.format(prefix)).astype(np.bool)
    user_adj_train = scipy.sparse.load_npz('./{}/{}/user_adj_train.npz'.format(prefix,fold)).astype(np.bool)
    user_role = json.load(open('./{}/{}/user_role.json'.format(prefix,fold)))
    user_feats = np.load('./{}/user_feats.npy'.format(prefix))
    # ---- normalize feats ----
    train_nodes = np.array(list(set(user_adj_train.nonzero()[0])))
    train_feats = user_feats[train_nodes]
    scaler = StandardScaler()
    scaler.fit(train_feats)
    user_feats = scaler.transform(user_feats)
    # -------------------------
    return user_adj_full, user_adj_train, user_feats, user_role
...
def data_prepare(flags):
    temp_data = load_text_data(flags.data_prefix, flags.fold)
    text_train_data = process_graph_data(*temp_data)
    print("Done loading training data of text..")
    user_train_data = load_user_data(flags.data_prefix, flags.fold)
    print("Done loading training data of users..")
    return text_train_data, user_train_data


def load_user_post_graph(prefix):
    user_post_graph = nx.read_edgelist('./{}/user_post_graph.txt'.format(prefix), create_using=nx.DiGraph())
    edges = nx.edges(user_post_graph)
    relation = {}
    for edge in edges:
        relation[int(edge[1])] = int(edge[0])
    return relation


def emb_concatenation(text_emb, user_emb, node_subgraph, flags):
    posting_relation = load_user_post_graph(flags.data_prefix)
    rows, cols = text_emb.shape
    num_users, dim_feats = user_emb.shape
    concate_emb = []
    for i in range(rows):
        if i in posting_relation.keys():
            u = posting_relation[i]
            if u in node_subgraph:
                u_index = np.where(node_subgraph == u)
                concate_emb.append(torch.add(text_emb[i], user_emb[u_index[0][0]], alpha=1))
                # concate_emb.append(torch.mul(text_emb[i], user_emb[u_index[0][0]]))
            else:
                concate_emb.append(text_emb[i])
        else:
            concate_emb.append(text_emb[i])
    concate_emb = torch.from_numpy(np.array([item.cpu().detach().numpy() for item in concate_emb]))
    return concate_emb
